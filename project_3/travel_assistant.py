'''
This deleberately mirrors the travel-planner example from Module 1,
...
It exercises every skill from both chapters:
    ...
    - a pre-build community tool (calculator)
    - a system prompt
    - the agentic loop doing multi-step planning
WHAT TO WATCH: the agent calls get_weather_forcast FIRST, feeds those numbers
...
def get_weather_forecast(city:str, days: int) -> dict:
    '''
        Get the weather forcast for a city over a number of days.
    '''
    forecasts = {
